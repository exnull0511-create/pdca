...
def run_strategy(races, mode="ev", ev_thresh=1.0, top_n=None, sim_thresh=None, n_sim=300):
    """
    mode="ev": EV>=ev_threshの買い目を全買い (top_n=Noneなら無制限、整数なら上限)
    mode="ev_sim": EV+シミュフィルタ(sim_thresh)
    mode="ev_topn_sim": EV>=thresh かつ EV順top_nを取り、シミュでフィルタ
    """
    rng=random.Random(42)
    total_inv=total_ret=total_n=total_hits=0
    bet_count_dist=Counter()
    ...
    for race in races:
        ...
        for sn in others:
            for tn in others:
                ...
                p_v=pl(axis,sn,tn)
                ev_v=p_v*o
                cands.append({"combo":c,"prob":p_v,"odds":o,"ev":ev_v})

        # EVフィルタ
        cands=[c for c in cands if c["ev"]>=ev_thresh]
        # EV順ソート
        ...
        # シミュフィルタ
        if sim_thresh is not None and cands:
            tri_counts=Counter()
            for _ in range(n_sim):
                r=simulate_race(race,rng,SIM_PARAMS)
                if len(r)>=3: tri_counts[f"{r[0]}-{r[1]}-{r[2]}"]+=1
            cands=[c for c in cands if tri_counts.get(c["combo"],0)>=sim_thresh]

        bets=[c["combo"] for c in cands]
        if not bets: continue
        bet_count_dist[len(bets)]+=1
    ...
    avg_bets = sum(k*v for k,v in bet_count_dist.items())/sum(bet_count_dist.values()) if bet_count_dist else 0
...
def cv_eval(name, **kw):
    ra=run_strategy(all_races,**kw)
    rv=run_strategy(test,**kw)
    fr=[run_strategy(folds[fi],**kw) for fi in range(3)]
    avg_cv=np.mean([f["roi"] for f in fr])
    return name, ra, rv, fr, avg_cv

configs = [
    # ベースライン (現行 = PL確率順 + シミュフィルタ閾値3 + 10点)
    ("現行 (PL順10点+シミュ3)", dict(mode="ev", ev_thresh=0, top_n=10, sim_thresh=3)),
    # EV>=1.0 だけで選定 (シミュなし、点数無制限)
    ("EV>=1.0 全買い (シミュなし)", dict(mode="ev", ev_thresh=1.0, top_n=None)),
    # EV>=1.0 + 上限あり
    ("EV>=1.0 上限7点", dict(mode="ev", ev_thresh=1.0, top_n=7)),
    ("EV>=1.0 上限10点", dict(mode="ev", ev_thresh=1.0, top_n=10)),
    ("EV>=1.0 上限15点", dict(mode="ev", ev_thresh=1.0, top_n=15)),
    # EV>=1.0 + シミュフィルタ
    ("EV>=1.0 上限10 + シミュ1", dict(mode="ev", ev_thresh=1.0, top_n=10, sim_thresh=1)),
    ("EV>=1.0 上限10 + シミュ3", dict(mode="ev", ev_thresh=1.0, top_n=10, sim_thresh=3)),
    # EV閾値変動
    ("EV>=1.5 上限10", dict(mode="ev", ev_thresh=1.5, top_n=10)),
    ("EV>=2.0 上限10", dict(mode="ev", ev_thresh=2.0, top_n=10)),
    ("EV>=3.0 上限10", dict(mode="ev", ev_thresh=3.0, top_n=10)),
]

print(f"\n{'='*120}")
print(f"{'設定':>30s}  {'R':>4s} {'平均点':>5s} {'的中':>4s} {'率':>5s} {'全ROI':>7s} {'全収支':>10s} {'検ROI':>7s} {'検収支':>10s} {'F1':>5s} {'F2':>5s} {'F3':>5s} {'CV平均':>6s}")
print(f"{'='*120}")

for name, kw in configs:
    n, ra, rv, fr, cv = cv_eval(name, **kw)
    if ra["n"]==0:
        print(f"  {name:>28s}  対象0件")
        continue
    hr = ra["hits"]/ra["n"]*100
    print(f"  {name:>28s}  {ra['n']:4d} {ra['avg_bets']:5.1f} {ra['hits']:4d} {hr:4.1f}% "
