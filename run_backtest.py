...
run_backtest.py
===============
racecard_hist.xlsx / odds_hist.xlsx / payouts_hist.xlsx を使って
本番同一ロジックでバックテストを実行する。期間フィルタ対応。

使い方:
  python run_backtest.py                                       # 全期間
  python run_backtest.py --start 2026-01-01 --end 2026-01-31  # 1月のみ
  python run_backtest.py --strategy PROD_3STAR                 # 本番☆☆☆設定
  python run_backtest.py --all-strategies                      # 全戦略一括
"""
...
def run_backtest(strategy: str, use_bank_filter: bool,
    ...
    if results:
        ...
        out_path = Path(f"data/backtest_hist_{strategy.lower()}.csv")
