...
======================
ハイブリッドDB方式バックテスト:
  旧DB（究極DB）= 選手の地力ベースライン
  slim DB       = 直近の調子シグナル
両方を融合して特徴量を算出する。

使い方:
  python run_backtest_hybrid.py --strategy LOOSE_A
  python run_backtest_hybrid.py --all-strategies
  python run_backtest_hybrid.py --strategy LOOSE_A --base-weight 0.5
"""
...
# ── ファイルパス ──────────────────────────────────────────────────────────────
RC_PATH  = Path("data/racecard_hist.xlsx")
OD_PATH  = Path("data/odds_hist.xlsx")
PY_PATH  = Path("data/payouts_hist.xlsx")
OUT_CSV  = Path("data/backtest_hist_result.csv")
...
def analyze_race(race_id, venue, race_dt, race_info, odds_dict,
    ...
    for _, row in race_info.iterrows():
        ...
        # ── ハイブリッドDB: 旧DB=地力ベースライン、slim=直近の調子 ──
        hist_old  = past_all[past_all["選手名_norm"] == nm] if not past_all.empty else pd.DataFrame()
        hist_slim = past_slim[past_slim["選手名_norm"] == nm] if not past_slim.empty else pd.DataFrame()

        BASE_W = cfg.get("base_weight", 0.4)   # 旧DBの重み（地力）
        RECENT_W_RATIO = 1.0 - BASE_W           # slim DBの重み（直近）

        ip = ep = 4.0; dp = bp_v = 3.0; nb = sp = 2.0; is_m = is_u = False

        def wm_old(hist, col):
            """旧DB: 全期間を均等重みで平均（地力ベースライン）"""
            if hist.empty or col not in hist.columns:
                return None
            v = pd.to_numeric(hist[col], errors="coerce")
            mk = v.notna()
            return float(v[mk].mean()) if mk.any() else None

        def wm_recent(hist, col):
            """slim DB: 直近2開催を重み3倍（調子シグナル）"""
            if hist.empty or col not in hist.columns:
                return None
            v = pd.to_numeric(hist[col], errors="coerce")
            sd = sorted(hist["開催日"].dropna().unique(), reverse=True)
            rd = set(sd[:2])
            w = np.where(hist["開催日"].isin(rd), 3.0, 1.0)
            mk = v.notna()
            return float((v[mk] * w[mk]).sum() / w[mk].sum()) if mk.any() else None

        def hybrid(col, default):
            """旧DBと slim DB を融合"""
            val_base   = wm_old(hist_old, col)
            val_recent = wm_recent(hist_slim, col)
            if val_base is not None and val_recent is not None:
                return val_base * BASE_W + val_recent * RECENT_W_RATIO
            elif val_recent is not None:
                return val_recent      # slim のみ
            elif val_base is not None:
                return val_base        # 旧DB のみ
            return default
        ...
        # 直線の伸び: slim は数値化済み列、旧DBは文字列列
        def _wm_scored(hist, col, score_fn, recent=False):
            """apply(score_fn) してから加重平均"""
            if hist.empty or col not in hist.columns:
                return None
            vals = hist[col].apply(score_fn).astype(float)
            mk = vals.notna()
            if not mk.any():
                return None
            if recent:
                ...
                rd = set(sd[:2])
                w = np.where(hist["開催日"].isin(rd), 3.0, 1.0)
                return float((vals[mk] * w[mk]).sum() / w[mk].sum())
            return float(vals[mk].mean())

        nb_slim = _wm_scored(hist_slim, "直線の伸び", nobi_score, recent=True)
        nb_old  = _wm_scored(hist_old,  nobi_col,     nobi_score, recent=False)
        if nb_old is not None and nb_slim is not None:
            nb = nb_old * BASE_W + nb_slim * RECENT_W_RATIO
        elif nb_slim is not None:
            nb = nb_slim
        elif nb_old is not None:
            nb = nb_old

        # 戦法
        sp_slim = _wm_scored(hist_slim, "戦法", senpo_lead, recent=True)
        sp_old  = _wm_scored(hist_old,  "戦法", senpo_lead, recent=False)
        if sp_old is not None and sp_slim is not None:
            sp = sp_old * BASE_W + sp_slim * RECENT_W_RATIO
        elif sp_slim is not None:
            sp = sp_slim
        elif sp_old is not None:
            sp = sp_old

        # is_monster / is_unreliable: どちらかのDBで検出されたらTrue
        if not hist_slim.empty:
            is_m = bool(hist_slim.get("is_monster",   pd.Series([0])).max() >= 1)
            is_u = bool(hist_slim.get("is_unreliable", pd.Series([0])).max() >= 1)
        if not is_m and not hist_old.empty:
            cmt = " ".join(hist_old.get("解析コメント", pd.Series([""])).astype(str))
            is_m = is_m or any(k in cmt for k in ["脚余し", "鬼脚", "別次元", "圧倒"])
            is_u = is_u or any(k in cmt for k in ["共倒れ", "位置取り失敗", "不発", "失速"])
...
def run_backtest(strategy: str, use_bank_filter: bool,
    ...
    bw = cfg.get("base_weight", 0.4)
    print(f"\n🎮 ストラテジー: {strategy} — {cfg['name']}")
    print(f"   バンクフィルタ: {'有効' if use_bank_filter else '無効'}")
    print(f"   ハイブリッドDB: 旧DB(地力)={bw:.0%} / slim(直近)={1-bw:.0%}")
    ...
    if start_date:
        ...
        rc_df = rc_df[rc_df["date"] >= sd]
        od_df = od_df[od_df["race_id"].isin(rc_df["race_id"].unique())]
        py_df = py_df[py_df["race_id"].isin(rc_df["race_id"].unique())]
    if end_date:
        ed = pd.Timestamp(end_date)
        rc_df = rc_df[rc_df["date"] <= ed]
        od_df = od_df[od_df["race_id"].isin(rc_df["race_id"].unique())]
        py_df = py_df[py_df["race_id"].isin(rc_df["race_id"].unique())]

    dates = sorted(rc_df["date"].dropna().unique())
    if not len(dates):
        print("⚠️  対象期間にデータがありません")
        return
    print(f"\n📅 バックテスト期間: {dates[0].date()} 〜 {dates[-1].date()}  ({len(dates)}日間)")
    print(f"   対象レース数: {rc_df['race_id'].nunique()} R")
    ...
    print(f"\n{'='*60}")
    print(f"🏁 【バックテスト結果】 {strategy}  ({period})")
    print(f"   {cfg['name']}")
    print(f"{'='*60}")
    ...
    if results:
        ...
        out_path = Path(f"data/backtest_hybrid_{strategy.lower()}.csv")
        df_res.to_csv(out_path, index=False, encoding="utf-8-sig")
        print(f"\n  → {out_path} 保存完了")


def main():
    parser = argparse.ArgumentParser(description="バックテスト実行（任意期間対応）")
    ...
    parser.add_argument("--start",          default=None,
                        help="開始日 YYYY-MM-DD（省略時=データ先頭）")
    parser.add_argument("--end",            default=None,
                        help="終了日 YYYY-MM-DD（省略時=データ末尾）")
    parser.add_argument("--base-weight",    default=0.4, type=float,
                        help="旧DB（地力）の重み 0.0〜1.0（デフォルト=0.4）")
    args = parser.parse_args()

    # base_weight を全ストラテジーに注入
    for cfg in STRATEGY_CONFIGS.values():
        cfg["base_weight"] = args.base_weight

    bf = not args.no_bank_filter
    if args.production:
        run_backtest("PRODUCTION", False, args.start, args.end)
        run_backtest("PROD_3STAR", True, args.start, args.end)
    elif args.all_strategies:
        for s in STRATEGY_CONFIGS:
            run_backtest(s, bf, args.start, args.end)
    else:
        run_backtest(args.strategy, bf, args.start, args.end)
