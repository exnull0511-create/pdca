"""閾値3+10点+消耗大 の検証 + ライン3番手の物理的不利を組み込む"""
...
# バンク直線距離 (メートル)
# 333mバンク: 直線約42m, 400mバンク: 直線約56m, 500mバンク: 直線約63m
BANK_STRAIGHT = {
    "前橋": 46.0, "宇都宮": 63.3, "豊橋": 60.3, "岸和田": 56.2,
    "熊本": 56.0, "いわき平": 62.7, "広島": 57.9, "別府": 59.9,
    "松山": 58.6, "小倉": 56.9, "京王閣": 51.5, "立川": 58.0,
    "取手": 54.8, "伊東": 46.6, "久留米": 50.7, "奈良": 38.2,
    "岐阜": 56.0, "小松島": 55.5, "防府": 42.5, "静岡": 56.4,
    "松阪": 61.5, "高知": 52.0, "松戸": 38.2, "平塚": 54.2,
    "西武園": 47.6, "小田原": 36.0, "大垣": 56.0, "名古屋": 58.8,
    "川崎": 58.0, "大宮": 46.7, "和歌山": 59.9, "玉野": 47.9,
    "武雄": 64.4, "四日市": 62.4, "岸和田": 56.2,
}
...
for race_date in dates:
    ...
    for race_id in daily_rc["race_id"].unique():
        ...
        straight_len = BANK_STRAIGHT.get(venue, 54.0)  # デフォルト54m
...
def simulate_race(race, rng, params):
    ...
    straight_len = race["straight_len"]
    # 直線が長いバンクほど後方からの差しが効く (54mを基準に正規化)
    straight_factor = straight_len / 54.0
    ...
    for num,p in players.items():
        ...
        if p["pos"]==1:  # ライン先頭
            ...
            # 物理的不利: 先頭から2人分後ろにいる
            # 直線で追い抜くには、nb(伸び)が高く、かつ直線が長いバンクでないと厳しい
            position_penalty = (p["pos"] - 2) * params["pos3_penalty"]
            sc += p["ep"] * 0.5 - position_penalty
    ...
    for num,p in players.items():
        ...
        # 直線の伸び: バンク直線距離が長いほど効く
        if p["pos"] >= 3:
            # 3番手以降: 伸びが活きるには直線が必要
            f += p["nb"] * params["w_nb"] * straight_factor * params["pos3_nb_boost"]
        else:
            f += p["nb"] * params["w_nb"]
        ...
        li=next((l for l in line_info if num in l["members"]),None)
        if li and p["pos"]==1:
            remaining_legs = 1.0  # 1.0 = フル、0 = 空
            if li==front:
                remaining_legs -= fl_fat * params["w_fat_straight"]
            elif li==rear:
                remaining_legs -= rl_fat * params["w_fat_straight"]
            # 脚が残ってないと直線で沈む
            f += (remaining_legs - 0.5) * params["w_remaining"]
...
# ── Parameters ───────────────────────────────────────────────────────────────
BASE_PARAMS = {
    "fat_front": 0.6, "fat_rear": 0.5,  # 消耗大
    "w_base": 0.3, "w_fatigue": 2.0,
    "w_dp_mid": 1.5, "ep_thresh": 4.5, "chigire_rate": 0.3, "chigire_penalty": 5.0,
    "w_bp_front": 0.8, "w_monster": 2.0,
    "w_nb": 1.5, "w_fat_straight": 1.0, "noise": 2.0,
    # 新パラメータ
    "pos3_penalty": 1.0,     # 3番手の位置ペナルティ
    "pos3_nb_boost": 1.0,    # 3番手の直線ブースト係数
    "w_remaining": 2.0,      # 残脚の直線への影響
}
...
# === 1. ベースライン(閾値3+10点+消耗大) ===
print(f"\n{'='*85}")
print("=== 1. ベースライン ===")
r_all=run_hybrid_filter(all_races, top_n=10, sim_threshold=3, params=BASE_PARAMS)
r_test=run_hybrid_filter(test, top_n=10, sim_threshold=3, params=BASE_PARAMS)
print(f"  全体: {r_all['n']}R  的中{r_all['hits']}({r_all['hits']/r_all['n']*100:.1f}%)  ROI {r_all['roi']:.1f}%  収支{r_all['profit']:+,}")
print(f"  検証: {r_test['n']}R  的中{r_test['hits']}  ROI {r_test['roi']:.1f}%  収支{r_test['profit']:+,}")

# === 2. 3番手パラメータ調整 ===
print(f"\n{'='*85}")
print("=== 2. 3番手の物理的不利 + 直線距離の影響 ===")
print(f"{'='*85}")
print(f"{'設定':>35s}  {'全ROI':>7s} {'全収支':>10s} {'検ROI':>7s} {'検収支':>10s}")
print("-"*75)

tweaks = [
    ("ベースライン", {}),
    ("3番手ペナルティ小 0.5", {"pos3_penalty": 0.5}),
    ("3番手ペナルティ大 2.0", {"pos3_penalty": 2.0}),
    ("3番手ペナルティ大 3.0", {"pos3_penalty": 3.0}),
    ("3番手直線ブースト 1.5", {"pos3_nb_boost": 1.5}),
    ("3番手直線ブースト 2.0", {"pos3_nb_boost": 2.0}),
    ("ペナ2.0+ブースト1.5", {"pos3_penalty": 2.0, "pos3_nb_boost": 1.5}),
    ("ペナ2.0+ブースト2.0", {"pos3_penalty": 2.0, "pos3_nb_boost": 2.0}),
    ("残脚影響大 3.0", {"w_remaining": 3.0}),
    ("残脚影響大 4.0", {"w_remaining": 4.0}),
    ("残脚+ペナ2.0+ブースト1.5", {"w_remaining": 3.0, "pos3_penalty": 2.0, "pos3_nb_boost": 1.5}),
    # ノイズ調整
    ("noise=1.5", {"noise": 1.5}),
    ("noise=2.5", {"noise": 2.5}),
    # 消耗さらに大
    ("消耗さらに大 0.8/0.7", {"fat_front": 0.8, "fat_rear": 0.7}),
]

for name, changes in tweaks:
    p = {**BASE_PARAMS, **changes}
    ra = run_hybrid_filter(all_races, top_n=10, sim_threshold=3, params=p)
    rv = run_hybrid_filter(test, top_n=10, sim_threshold=3, params=p)
    if ra["n"]==0: continue
    print(f"  {name:>33s}  {ra['roi']:6.1f}% {ra['profit']:>+10,} {rv['roi']:6.1f}% {rv['profit']:>+10,}")

# === 3. 3分割CV ===
print(f"\n{'='*85}")
print("=== 3. 3分割CV (最有望な設定) ===")
print(f"{'='*85}")

best_configs = [
    ("ベースライン(閾値3+10点+消耗大)", {}),
    ("ペナ2.0+ブースト1.5", {"pos3_penalty": 2.0, "pos3_nb_boost": 1.5}),
    ("残脚3.0+ペナ2.0+ブースト1.5", {"w_remaining": 3.0, "pos3_penalty": 2.0, "pos3_nb_boost": 1.5}),
]
...
for name, changes in best_configs:
    p = {**BASE_PARAMS, **changes}
    fold_rois = []
    for fi in range(3):
        rv = run_hybrid_filter(folds[fi], top_n=10, sim_threshold=3, params=p)
        fold_rois.append(rv["roi"])
    avg = np.mean(fold_rois)
    print(f"  {name:>40s}  F1:{fold_rois[0]:5.1f}%  F2:{fold_rois[1]:5.1f}%  F3:{fold_rois[2]:5.1f}%  平均:{avg:5.1f}%")
