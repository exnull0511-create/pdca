...
======================
3月のデータ (racecard_march.xlsx等) を統合し、
既存のロック条件（LOOSE_B等）およびEV傾斜配分を適用して
simulation_model.py のシミュレーションを用いたバックテストを実行する。
"""
...
SCFG = {
    "name":             "[SIM] skip_chaos=True / min_top_ev=70 / EV傾斜14点",
    "skip_chaos":       True,
    "min_top_ev":       70,
...
def build_race_data_for_sim(race_info, past_db, past_slim, nobi_col):
    ...
    for _, row in race_info.iterrows():
        ...
        # bank特性等省いた簡易EV(しきい値70に乗るようにやや高め)
...
def ev_alloc(bet_ev_list, bet_base):
    # bet_ev_list: [{'bet': '1-2-3', 'prob':0.05, 'odds': 100.0, 'ev': 5.0}, ...]
...
def main():
    print(f"🚀 3月データ バックテスト (Simulation Model + ロック条件) を開始...")
    print(f"   {SCFG['name']}")
    ...
    rc_df = rc_df[rc_df['date'] >= pd.Timestamp('2026-03-01')]
    ...
    if not dates:
        print("💡 対象期間（3月）のデータがありませんでした。処理を終了します。")
        return
        
    print(f"対象期間: {dates[0].date()} 〜 {dates[-1].date()}")
    print(f"対象レース数: {len(unique_races)}R\n")
    
    results = []
    skipped = {'no_data': 0, 'low_bank': 0, 'low_ev': 0, 'chaos': 0, 'no_monster': 0}
    total_invest = 0
    total_return = 0
    hit_count = 0
    bet_races = 0
    ...
    for race_date in dates:
        ...
        for race_id in daily_rc['race_id'].unique():
            ...
            # フィルタ: 低bank除外
            if SCFG['skip_low_bank'] and BANK_DICT.get(venue, {}).get('roi_tier') == 'low':
                skipped['low_bank'] += 1
                continue
            ...
            if SCFG['skip_chaos'] and is_chaos:
                skipped['chaos'] += 1
                continue
                
            # フィルタ: 鬼脚必須
            has_monster = any(sc['is_monster'] for sc in player_scores.values())
            if SCFG['require_monster'] and not has_monster:
                skipped['no_monster'] += 1
                continue
            ...
            ev_list = simulation_model.calculate_expected_value(probs, odds_dict, top_n=SCFG['top_n_prob_bets'])
            if not ev_list:
                skipped['no_data'] += 1
                continue
            ...
            if (bet_races % 50) == 0:
                print(f"  ... 処理 {bet_races} レース経過 (的中 {hit_count})")
                
            results.append({
                'date': str(race_date.date()), 'venue': venue, 'race_no': race_no,
                'race_id': race_id, 'invest': invest, 'return': ret,
                'hit': hit, 'actual': actual, 'payout': payout,
            })
    ...
    print(f"\n{'='*60}")
    print(f"🏁 【SIMモデル ロック条件＋EV配分 (3月)】")
    print(f"{'='*60}")
    print(f"  実行時間      : {elapsed:.1f} 秒")
    print(f"  対象レース数  : {bet_races+sum(skipped.values()):>5} R")
    print(f"  スキップ合計  : {sum(skipped.values()):>5} R")
    print(f"   (低bank:{skipped['low_bank']} EV不足:{skipped['low_ev']} カオス:{skipped['chaos']})")
    print(f"  ────────────────────────────")
    ...
    if results:
        pd.DataFrame(results).to_csv("data/backtest_sim_march_locked.csv", index=False, encoding='utf-8-sig')
        print("📁 結果を data/backtest_sim_march_locked.csv に保存しました。")
